...
use crate::arrow::arrow_reader::selection::{
    RowSelectionBacking, RowSelectionPolicy, RowSelectionStrategy, mask_to_selectors,
};
...
fn build_cursor(selection: RowSelection, strategy: RowSelectionStrategy) -> RowSelectionCursor {
    match (strategy, selection.into_backing()) {
        (RowSelectionStrategy::Mask, RowSelectionBacking::Mask(mask)) => {
            RowSelectionCursor::new_mask_from_buffer(mask)
        }
        (RowSelectionStrategy::Mask, RowSelectionBacking::Selectors(selectors)) => {
            RowSelectionCursor::new_mask_from_selectors(selectors)
        }
        (RowSelectionStrategy::Selectors, RowSelectionBacking::Selectors(selectors)) => {
            RowSelectionCursor::new_selectors(selectors)
        }
        (RowSelectionStrategy::Selectors, RowSelectionBacking::Mask(mask)) => {
