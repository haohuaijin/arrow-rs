...
pub struct RowSelection {
    backing: RowSelectionBacking,
}

/// Internal storage for [`RowSelection`].
#[derive(Debug, Clone)]
pub(crate) enum RowSelectionBacking {
    Selectors(Vec<RowSelector>),
    Mask(BooleanBuffer),
}

impl Default for RowSelectionBacking {
    fn default() -> Self {
...
impl std::fmt::Debug for RowSelection {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self.backing {
            RowSelectionBacking::Selectors(s) => f
                .debug_struct("RowSelection")
                .field("selectors", s)
                .finish(),
            RowSelectionBacking::Mask(m) => f
                .debug_struct("RowSelection")
...
impl PartialEq for RowSelection {
    fn eq(&self, other: &Self) -> bool {
        match (&self.backing, &other.backing) {
            (RowSelectionBacking::Selectors(a), RowSelectionBacking::Selectors(b)) => a == b,
            (RowSelectionBacking::Mask(a), RowSelectionBacking::Mask(b)) => a == b,
...
impl RowSelection {
    fn from_selectors(selectors: Vec<RowSelector>) -> Self {
        Self {
            backing: RowSelectionBacking::Selectors(selectors),
        }
    }
    ...
    pub fn from_boolean_buffer(mask: BooleanBuffer) -> Self {
        Self {
            backing: RowSelectionBacking::Mask(mask),
        }
    }
    ...
    pub fn as_mask(&self) -> Option<&BooleanBuffer> {
        match &self.backing {
            RowSelectionBacking::Mask(m) => Some(m),
            _ => None,
        }
    }

    /// Consume the selection and return its internal storage.
    pub(crate) fn into_backing(self) -> RowSelectionBacking {
        self.backing
    }
    ...
    fn into_selectors_vec(self) -> Vec<RowSelector> {
        match self.backing {
            RowSelectionBacking::Selectors(s) => s,
            RowSelectionBacking::Mask(m) => mask_to_selectors(&m),
        }
    }

    /// Promote a mask-backed selection to selector backing in place.
    fn selectors_mut(&mut self) -> &mut Vec<RowSelector> {
        if let RowSelectionBacking::Mask(_) = &self.backing {
            let mask = match std::mem::take(&mut self.backing) {
                RowSelectionBacking::Mask(m) => m,
                RowSelectionBacking::Selectors(_) => unreachable!(),
            };
            self.backing = RowSelectionBacking::Selectors(mask_to_selectors(&mask));
        }
        match &mut self.backing {
            RowSelectionBacking::Selectors(s) => s,
            RowSelectionBacking::Mask(_) => unreachable!(),
        }
    }
    ...
    pub fn split_off(&mut self, row_count: usize) -> Self {
        if matches!(&self.backing, RowSelectionBacking::Mask(_)) {
            let mask = match std::mem::take(&mut self.backing) {
                RowSelectionBacking::Mask(m) => m,
                RowSelectionBacking::Selectors(_) => unreachable!(),
            };
            ...
            self.backing = RowSelectionBacking::Mask(tail);
    ...
    fn materialize_for_combine(&self) -> Vec<RowSelector> {
        match &self.backing {
            RowSelectionBacking::Selectors(s) => s.clone(),
            RowSelectionBacking::Mask(m) => mask_to_selectors(m),
        }
    }
    ...
    pub fn selects_any(&self) -> bool {
        match &self.backing {
            RowSelectionBacking::Selectors(s) => s.iter().any(|x| !x.skip),
            RowSelectionBacking::Mask(m) => m.set_indices().next().is_some(),
        }
    }

    /// Trims this [`RowSelection`] removing any trailing skips
    pub(crate) fn trim(mut self) -> Self {
        if let RowSelectionBacking::Mask(m) = &self.backing {
            let len = m.len();
            let mut new_len = len;
            while new_len > 0 && !m.value(new_len - 1) {
                new_len -= 1;
            }
            if new_len != len {
                return Self {
                    backing: RowSelectionBacking::Mask(m.slice(0, new_len)),
                };
            }
            return self;
        }
    ...
    pub fn iter(&self) -> RowSelectionIter<'_> {
        match &self.backing {
            RowSelectionBacking::Selectors(s) => RowSelectionIter::Selectors(s.iter().copied()),
            RowSelectionBacking::Mask(m) => RowSelectionIter::Mask(MaskRunIter::new(m)),
        }
    }
    ...
    pub fn row_count(&self) -> usize {
        match &self.backing {
            RowSelectionBacking::Selectors(s) => {
                s.iter().filter(|x| !x.skip).map(|x| x.row_count).sum()
            }
            RowSelectionBacking::Mask(m) => m.count_set_bits(),
        }
    }
    ...
    pub fn skipped_row_count(&self) -> usize {
        match &self.backing {
            ...
            RowSelectionBacking::Mask(m) => m.len() - m.count_set_bits(),
        }
    }
...
impl FromIterator<RowSelection> for RowSelection {
    ...
    fn from_iter<T: IntoIterator<Item = RowSelection>>(iter: T) -> Self {
        ...
        let all_mask = items
            .iter()
            .all(|s| matches!(&s.backing, RowSelectionBacking::Mask(_)));

        if all_mask {
            let total_len: usize = items
                .iter()
                .map(|s| match &s.backing {
                    RowSelectionBacking::Mask(m) => m.len(),
                    RowSelectionBacking::Selectors(_) => unreachable!(),
                })
                .sum();
            let mut builder = BooleanBufferBuilder::new(total_len);
            for item in items {
                match item.into_backing() {
                    RowSelectionBacking::Mask(m) => builder.append_buffer(&m),
                    RowSelectionBacking::Selectors(_) => unreachable!(),
                }
            }
...
mod tests {
    ...
    fn test_mask_backing_fuzz_equivalence() {
        ...
        for _ in 0..100 {
            ...
            assert_eq!(from_mask.and_then(&inner), from_filters.and_then(&inner));
        }
    }
