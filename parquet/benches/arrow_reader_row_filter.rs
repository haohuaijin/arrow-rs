...
/// Total number of rows written to the in-memory parquet file used by the
/// benchmarks.
const TOTAL_ROWS: usize = 500_000;

/// Maximum rows per row group. With [`TOTAL_ROWS`] = 500K and this set to
/// 100K, the benchmark file contains 5 row groups — enough to exercise
/// Layer 1 (skip later row groups once the output limit is exhausted) in
/// the `LIMIT` bench, on top of Layer 2's within-row-group short-circuit.
